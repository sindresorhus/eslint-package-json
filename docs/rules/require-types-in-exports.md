...
It also performs static checks that type targets use `.d.ts`, `.d.mts`, or `.d.cts`, and that those extensions agree with structurally matching `.mjs`, `.cjs`, and package-`type` formats. Branch and fallback analysis assumes structurally valid `exports` targets; use [`valid-fields`](valid-fields.md) to report malformed targets. Coverage through declaration defaults is best-effort, and module-format checks do not follow a nested `types` or `types@` condition into a later declaration `default`. Model fallback branches with direct runtime conditions when they need complete static validation. The rule does not analyze overlap or shadowing between multiple versioned conditions, or simulate custom TypeScript conditions. For target arrays, only the first entry is checked. The rule does not check whether declaration files exist, are published, or contain matching declarations. Use package-aware tools for those checks.
