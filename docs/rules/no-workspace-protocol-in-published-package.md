...
This rule checks source `package.json` files, so it disallows `workspace:` specifiers in every package that can be published, even if release tooling could rewrite them. Projects that intentionally rely on that rewrite should disable this rule for those source manifests. Packages with `"private": true` are exempt because npm refuses to publish them.
