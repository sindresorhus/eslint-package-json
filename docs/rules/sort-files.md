...
This rule puts exact paths referenced by `exports`, legacy entry-point fields, and `bin` first. Runtime files come before their `.d.ts`, `.d.mts`, or `.d.cts` declarations. Remaining paths, directories, and glob patterns are ordered by path using the same declaration pairing.
