...
| [prefer-side-effects-field](docs/rules/prefer-side-effects-field.md)                                   | Recommend declaring the `sideEffects` field for packages with an `exports` field and bundler conditions in `exports` or `imports`. | ✅  |    |    |
